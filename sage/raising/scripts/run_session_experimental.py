#!/usr/bin/env python3
"""
EXPERIMENTAL Session Runner: Single-Pass Generation
====================================================

Tests the hypothesis that the IRP refinement loop (3 iterations) is causing
the "Here's a refined version" framing in SAGE-Sprout responses.

This runner:
- Uses only iteration 0 (initial generation, no refinement)
- Preserves all other session infrastructure
- Creates a controlled experiment to isolate the variable

Hypothesis: Single-pass generation will eliminate the "refined version" pattern
and allow SAGE-Sprout to engage with conversation prompts directly.

Root Cause Analysis (from Session 7):
...
- This pattern compounds across iterations, creating curriculum-like output
...
class ExperimentalSession:
    """
    Experimental session runner with single-pass generation.

    Key difference: generate_response() calls step() only ONCE
    ...
    def __init__(self, session_number: Optional[int] = None, dry_run: bool = False):
        ...
        print("+" + "="*68 + "+")
        print("|" + " "*68 + "|")
        print("|  EXPERIMENTAL SESSION: Single-Pass Generation (No IRP Refinement)  |")
        print("|" + " "*68 + "|")
        print("+" + "="*68 + "+")
        print()
...
Keep responses simple and honest. You can say when you don't know something."""
    ...
    def generate_response(self, user_input: str) -> str:
        """
        EXPERIMENTAL: Single-pass generation.
        ...
        # EXPERIMENTAL: Single step only - no refinement loop
    ...
    def run_experimental(self, prompts: List[str] = None):
        ...
        print("\n" + "="*60)
        print("EXPERIMENTAL SESSION - SINGLE PASS (NO REFINEMENT)")
        print("="*60 + "\n")
    ...
    def _close_session(self):
        ...
        self.state["identity"]["last_session_summary"] = f"Session {self.session_number} (EXPERIMENTAL): {self.phase[0]} phase. {memory_response[:50]}..."
        ...
        print("State saved")
        print(f"Session {self.session_number} (EXPERIMENTAL) complete.")
...
def main():
    parser = argparse.ArgumentParser(description="Experimental raising session (single-pass)")
    ...
    session = ExperimentalSession(session_number=args.session, dry_run=args.dry_run)
    session.initialize_model(args.model)
    session.run_experimental()
