#!/usr/bin/env python3
"""
McNugget Raising Session Runner
================================

SAGE-McNugget raising via OllamaIRP — no torch, no CUDA, no Qwen.
Uses the same BECOMING_CURRICULUM as Sprout but through Ollama's HTTP API,
targeting Gemma 3 12B on McNugget (Mac Mini M4, 16GB unified).
...
class McNuggetRaisingSession:
    ...
    STATE_FILE = RAISING_DIR / "state" / "mcnugget_identity.json"
    SESSIONS_DIR = RAISING_DIR / "sessions" / "mcnugget"
    IDENTITY_DIR = HRM_ROOT / "sage" / "identity"
    EXPERIENCE_BUFFER = RAISING_DIR / "state" / "experience_buffer_mcnugget_gemma3_12b.json"
