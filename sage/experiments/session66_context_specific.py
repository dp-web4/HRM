#!/usr/bin/env python3
"""
Session 66: Context-Specific Trust Learning

Goal: Enable context-aware trust evolution (code/reasoning/text)

Building on Session 65:
- Session 65 validated feedback loop works ✅
- But all sequences used "general" context
- No cross-sequence learning or context differentiation

What's New in Session 66:
- **Context Labels**: Assign semantic context to each sequence
  * "code" - fibonacci, DataProcessor
  * "reasoning" - quantum mechanics, consciousness
  * "text" - once upon, weather
- **Context-Specific Trust**: update_context_trust(context="code"|"reasoning"|"text")
- **Cross-Context Analysis**: Does trust in one context transfer to others?
- **Context Convergence**: Which contexts learn faster?

Method:
1. Label 6 sequences with semantic contexts (2 per context type)
2. Baseline: Router-only (unchanged, for comparison)
3. Trust-augmented: Router + Trust with CONTEXT-SPECIFIC feedback
   - After each generation: record_quality(expert_id, context=actual_context, quality)
   - Trust evolves per-context (not global)
4. Track trust evolution for each context separately
5. Analyze cross-context patterns and convergence rates

Expected Outcomes:
- Context-specific trust convergence (code trust ≠ text trust)
- Cross-context learning insights (does code help reasoning?)
- Foundation for real context classification (when embeddings available)
- More accurate expert selection (context-aware)

Web4 Connection:
- MRH (Minimal Resonance Hypothesis): Different contexts → different patterns
- Contextual trust more accurate than global
- Aligns with biological specialization (visual cortex vs language areas)
"""
...
def create_realistic_sequences():
    """
    Create realistic token sequences for testing WITH CONTEXT LABELS.

    NEW in Session 66: Each sequence now has a semantic context label.
    ...
    - **Context labels for context-specific trust**

    Returns:
        List of (input_ids, target_ids, prompt_text, context) tuples
    """
    sequences = []

    # Code sequences (lower token IDs, more structured) - CONTEXT: "code"
    sequences.extend([
        (
    ...
    # Reasoning sequences (mid-range token IDs, more abstract) - CONTEXT: "reasoning"
    sequences.extend([
        (
    ...
    # Text sequences (varied token IDs, more diverse) - CONTEXT: "text"
    sequences.extend([
        (
...
def run_baseline_real(extraction_dir, sequences, num_epochs=3):
    ...
    Args:
        ...
        sequences: List of (input, target, prompt, context) tuples  # NEW: 4-tuple
    ...
    for epoch in range(num_epochs):
        for idx, (input_ids, target_ids, prompt, context) in enumerate(sequences):  # NEW: Unpack context
            gen_num = epoch * len(sequences) + idx + 1
            print(f"Generation {gen_num}/{num_epochs * len(sequences)}: '{prompt[:30]}...' [{context}]", end="", flush=True)
...
def run_trust_augmented_real(extraction_dir, sequences, alpha=0.5, num_epochs=3):
    ...
    # NEW: Track trust per context
    context_trust_evolution = {"code": [], "reasoning": [], "text": []}
    ...
    for epoch in range(num_epochs):
        for idx, (input_ids, target_ids, prompt, context) in enumerate(sequences):  # NEW: Unpack context
            ...
            try:
                ...
                # **SESSION 66: Context-Specific Quality Feedback Loop**
                ...
                # **NEW: Update trust based on ACTUAL CONTEXT (code/reasoning/text)**
                rep.update_context_trust(context=context, evidence_quality=quality_score, learning_rate=0.2)

                # Track trust evolution PER CONTEXT
                trust = rep.get_context_trust(context, default=0.5)
                trust_evolution.append(trust)
                context_trust_evolution[context].append(trust)

                print(f" → PPL: {perplexity:.2f}, Q: {quality_score:.4f}, Trust[{context}]: {trust:.3f}")
    ...
    if qualities:
        ...
        print(f"\nTrust-Augmented Results (Overall):")
        print(f"  Average perplexity: {avg:.2f}")
        print(f"  Early (epoch 1): {early:.2f}")
        print(f"  Late (epoch {num_epochs}): {late:.2f}")
        print(f"  Improvement: {improvement:+.1f}%")
        print(f"  Trust evolution (all): {trust_evolution[0]:.3f} → {trust_evolution[-1]:.3f}")

        # **NEW: Context-Specific Analysis**
        ...
        for ctx, trust_values in context_trust_evolution.items():
            if trust_values:
                ...
                print(f"    [{ctx:10s}] {ctx_early:.3f} → {ctx_late:.3f} ({ctx_change:+.1f}% change, n={len(trust_values)})")

    return qualities, trust_evolution, context_trust_evolution
...
def main():
    """Main execution."""
    print("="*70)
    print("Session 66: Context-Specific Trust Learning")
    print("="*70)
    print("\nGoal: Enable context-aware trust evolution (code/reasoning/text)\n")
    ...
    print(f"\nPrepared {len(sequences)} realistic test sequences WITH CONTEXT LABELS:")
    print("  - Code sequences: 2 (fibonacci, DataProcessor)")
    print("  - Reasoning sequences: 2 (quantum, consciousness)")
    print("  - Text sequences: 2 (once upon, weather)")
    ...
    # Run trust-augmented (NOW RETURNS 3 VALUES)
    trust_qualities, trust_evolution, context_trust_evolution = run_trust_augmented_real(
    ...
    # Save results (NEW: Include context-specific trust)
    results = {
        'baseline': {
        ...
        'num_epochs': 3,
        'contexts': ['code', 'reasoning', 'text']  # NEW: Document contexts
    }

    output_file = Path(__file__).parent / "session66_results.json"
