#!/usr/bin/env python3
"""
Phase 2.1 Training - System Prompt Approach

Trains model to respond appropriately when SAGE provides context via system prompt.
This approach assumes SAGE will handle context classification and provide
appropriate system prompts to steer the LLM.
"""
...
def load_training_data(corpus_path):
    """Load Phase 2.1 corpus and format for system prompt training."""
    with open(corpus_path) as f:
        corpus = json.load(f)

    examples = []
    for item in corpus:
        # Format with system prompt context
        examples.append({
            "prompt": f"System: {item['system_prompt_context']}\nUser: {item['question']}",
...
def main():
    ...
    metadata = {
        "approach": "system_prompt_steering",
        "description": "SAGE provides context via system prompt",
