...
def test_qwen3_omni():
    ...
    print("Loading model (66GB, this will take a moment)...")
    ...
    response1 = tokenizer.decode(outputs[0], skip_special_tokens=True).replace(prompt1, "").strip()
    ...
    outputs = model.generate(**inputs, max_new_tokens=200, temperature=0.7)
    response2 = tokenizer.decode(outputs[0], skip_special_tokens=True).replace(prompt2, "").strip()
