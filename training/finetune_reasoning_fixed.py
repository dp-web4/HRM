#!/usr/bin/env python3
"""
Fine-tune HRM for Real Reasoning on Non-Augmented ARC Tasks - FIXED VERSION
Key changes:
- Allow up to 20 reasoning cycles (vs 8)
- Train on original tasks only (no augmentation)
- FIXED: Proper halt predictor initialization
"""
import os
import sys
import torch
...
class HierarchicalReasoningModule(nn.Module):
    """HRM with FIXED enhanced reasoning cycles"""
    ...
    def __init__(self, config):
        ...
        # Simple halt predictor - compatible with original model
        # We'll enhance it carefully without breaking it
        self.halt_predictor = nn.Linear(config['hidden_size'] * 2, 1)
        
        # Additional gating for extended reasoning (new params that won't break loading)
        self.halt_gate = nn.Parameter(torch.tensor(0.1))  # Start with low gating
    ...
    def forward(self, x, max_cycles=None):
        ...
        for cycle in range(max_cycles):
            ...
            # FIXED: Apply gating and ensure gradual increase
            # This prevents immediate halting
            cycle_factor = (cycle + 1) / max_cycles  # 0.05 to 1.0 over 20 cycles
            gated_logit = halt_logit * self.halt_gate * cycle_factor
            halt_prob = torch.sigmoid(gated_logit)
            halt_probs.append(halt_prob)
            
            # Adaptive early stopping with minimum cycles
            if cycle >= 5:  # Minimum 5 cycles (was 3)
                # Only stop if very confident AND we've done enough cycles
                if halt_prob.mean() > 0.95 and cycle >= 8:
                    break
...
class ReasoningACTLoss(nn.Module):
    ...
    def __init__(self, halt_penalty=0.0001):  # Much lower penalty
        super().__init__()
    ...
    def forward(self, outputs, targets, halt_probs):
        ...
        if halt_probs:
            ...
            # Want to encourage 8-15 cycles on average
            cycle_penalty = 0.0
            if num_cycles < 5:  # Too few cycles
                cycle_penalty = (5 - num_cycles) * 0.1
            elif num_cycles > 18:  # Too many cycles (leave room for hard tasks)
                cycle_penalty = (num_cycles - 18) * 0.01
            
            # Average halt probability (encourage gradual increase, not immediate)
            halt_loss = 0.0
            for i, p in enumerate(halt_probs):
                # Earlier cycles should have lower halt probability
                target_prob = min(0.9, (i + 1) / len(halt_probs))
                halt_loss += (p.mean() - target_prob) ** 2
...
def finetune(dataset_name='agi-1'):
    ...
    print(f"🎯 Fine-tuning for reasoning on {dataset_name.upper()} - FIXED VERSION")
    print(f"📊 Max reasoning cycles: {MODEL_CONFIG['max_cycles']}")
    print("✅ Halt predictor initialization FIXED")
    print("=" * 60)
    ...
    print(f"✅ Halt predictor properly initialized from original weights")
    ...
    with torch.no_grad():
        ...
        print(f"  Initial halt probs: {halt_values}")
    ...
    # Loss function
    criterion = ReasoningACTLoss(halt_penalty=0.0001)
    
    # Initialize wandb
    run = wandb.init(
        project="hrm-reasoning-finetune-fixed",
        name=f"reasoning-{output_suffix}-fixed-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        config={**MODEL_CONFIG, **TRAINING_CONFIG, 'dataset': dataset_name},
        mode="offline"
    )
    
    # Training loop
    print("\n🏃 Starting fine-tuning for reasoning (FIXED)...")
    model.train()
    ...
    while global_step < TRAINING_CONFIG['num_steps']:
        for batch in train_loader:
            ...
            if global_step % TRAINING_CONFIG['checkpoint_frequency'] == 0 and global_step > 0:
                save_path = f'checkpoints/hrm_reasoning_{output_suffix}_fixed_step_{global_step}.pt'
                torch.save({
                ...
                with torch.no_grad():
                    ...
                    print(f"  Cycles at step {global_step}: {len(test_halt)}")
                model.train()
    ...
    # Save final model
    final_path = f'checkpoints/hrm_reasoning_{output_suffix}_fixed_final.pt'
    torch.save({
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'global_step': global_step,
    ...
    print(f"📊 Final average cycles: {accumulated_cycles / TRAINING_CONFIG['gradient_accumulation_steps']:.1f}")
    
    return final_path

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Fine-tune HRM for reasoning (FIXED)')
